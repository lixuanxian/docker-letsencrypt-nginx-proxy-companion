...
```bash
docker build -t kineviz/nginx-acme-companion .
test/run.sh kineviz/nginx-acme-companion
```

You can limit the test run to specific test(s) with the `-t` flag:

```bash
test/run.sh -t docker_api kineviz/nginx-acme-companion
```
...
```bash
test/run.sh -d kineviz/nginx-acme-companion
```
