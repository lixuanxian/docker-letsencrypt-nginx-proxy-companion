...
<picture>
  ...
  <source
    ...
    srcset="./docs/img/acme-companion_dark_theme.svg" />
  <!-- Light mode -->
  <source
    media="(prefers-color-scheme: light)"
    srcset="./docs/img/acme-companion_light_theme.svg" />
  <!-- Fallback for environments without media queries -->
  <img
    alt="ZeroSSL"
    src="./docs/img/acme-companion_light_theme.svg"
    height="auto" />
</picture>
</a>
...
[![GitHub release](https://img.shields.io/github/release/nginx-proxy/acme-companion.svg)](./releases)
...
* Let's Encrypt / ACME domain validation through `HTTP-01` (by default) or [`DNS-01`](./docs/Let's-Encrypt-and-ACME.md#dns-01-acme-challenge) challenge.
...
* Support creation of [IP address certificates](./docs/IP-address-certificates.md) (`HTTP-01` challenge only) from any CA that issues them, such as Let's Encrypt (short lived) and [Google Trust Services](./docs/Google-Trust-Services.md#ip-address-certificates).
...
* Your host **must** be publicly reachable on **both** port [`80`](https://letsencrypt.org/docs/allow-port-80/) and [`443`](./discussions/873#discussioncomment-1410225).
...
Please check the [docs section](./tree/main/docs) and the [environment variables reference](./docs/Environment-variables-reference.md) for more information about the available configuration options.
